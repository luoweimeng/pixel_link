...
def decode_image_by_join(pixel_scores, link_scores,
    ...
    updown_link_mask = link_scores >= updown_link_conf_threshold
    print(link_conf_threshold)
    print(updown_link_conf_threshold)
    ...
    for point in points:
        ...
        for n_idx, (nx, ny) in enumerate(neighbours):
            ...
            if is_valid_cord(nx, ny, w, h):
                # the neighbour is in the up or down direction, use the updown_link_conf_threshold
                # if n_idx in [0, 1, 2, 5, 6, 7]:
                #     link_value = updown_link_mask[y, x, n_idx]
                # else:
                ...
                if n_idx in [0, 1, 2, 5, 6, 7]:
                    link_value = updown_link_mask[y, x, n_idx]
                    pixel_cls = pixel_mask[ny, nx]
                    if pixel_cls and link_value and updown_link_mask[y, x, 1] and updown_link_mask[y, x, 6]:
                        join(point, (ny, nx))
                else:
                    #                 reversed_neighbours = get_neighbours(nx, ny)
                    #                 reversed_idx = reversed_neighbours.index((x, y))
